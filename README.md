...
Because of Github file size limit, we share the data storage directory that must be extracted into the root of this repository. You can download it from this [link](https://drive.google.com/file/d/1IvytQ8lJUbR7WRzMkY2aBnMWCkaviOJ0/view?usp=drive_link).

```bash
# make sure to install unzip and map to your repository root path
unzip data_storage.zip -d ../GREAT
```
...
Just like `/data_storage` size limitation, we have a `/recommendations` directory that serves datasets and essential group recommender results. You can download it from this [link](https://drive.google.com/file/d/1WNwj9zBuLLKDJI5HKKq_REONbq4GHcDw/view?usp=drive_link).

```bash
# make sure to install unzip and map to your repository root path
unzip recommendations.zip -d ../GREAT
```
...
Alternatively you can add your own dataset remapping new attributes into the GREAT API Service. And replacing `/recomendations/dataset/user_data` with your own ratings data with the following structure: `user_id,item_id,rating,timestamp`.

### GroupLibRec setup

Check the downloaded `/recomendations` directory and you will notice all needed GroupLibRec resources for input/output executed by the compiled Java `.jar` tool program. Specifically, `group.properties` and `user.properties` are configuration files used to execute the recommendation process. Those files contain GroupLibRec configuration propierties for reproducibility and tunning.
...
We made images available for the dataset we used. These images are served as static files. Download it from this [link](https://drive.google.com/file/d/1EoSwhz2LTY5X1jscjxYXirYQuzCttuKM/view?usp=drive_link)

```bash
# make sure to install unzip and map to ../server-grouplibrec root path
unzip images.zip -d ../GREAT/web-librec/server-grouplibrec
```
...
DIR_GROUPLIBREC=$BASE_DIR/recomendations/grouplibrec.jar
...
DIR_ICONS_USERS=$BASE_DIR/web-librec/server-grouplibrec/icons
...
```
Make sure directories and files exist within those environment variables so the API service can capture and use them.
