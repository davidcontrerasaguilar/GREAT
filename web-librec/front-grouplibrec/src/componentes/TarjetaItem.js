...
import { buttonRecommendContext } from "../context/ButtonContext"
...
const TarjetaItem = (props) => {
    const { t, i18n } = useTranslation("componentes/tarjeta_item")
                        ...
                        <div className="columns">
                            <div className="column">
                                <div className="is-flex-direction-row">
                                    <button id='indiv-rec-1' className={`button ${buttonRecommend? 'is-loading': ''}`} disabled={buttonRecommend} style={styleButtonItem} onClick={() => setDestinoEstado(true)}>
                                        <FontAwesomeIcon icon={faPeopleGroup} style={{ color: "#09b391" }} />
                                    </button>
                                    <button id='indiv-rec-2' className={`button ${buttonRecommend? 'is-loading': ''}`} disabled={buttonRecommend} style={styleButtonItem} onClick={() => props.enviarAlStack(itemId)}>
                                        <FontAwesomeIcon icon={faHeart} style={{ color: "#e41b43", }} />
                                    </button>
                                    <button id='indiv-rec-3' className={`button ${buttonRecommend? 'is-loading': ''}`} disabled={buttonRecommend} style={styleButtonItem} onClick={() => setModalEstado(true)}>
                    ...
                    <div className="column">
                        ...
                        <div className="columns">
                            <div className="column">
                                {props.item !== null &&
                                    <div>
                                        <p>{t('main.item.recommendedBy')}: <span className="has-text-weight-bold">{props.item.recomendadoPorUsuario}</span></p>
                                    </div>
                                }
                            </div>
                        </div>
                        <div className="columns">
                            <div className="column">
                                <div className="is-flex-direction-row">
                                    <button className={`button ${buttonRecommend? 'is-loading': ''}`} disabled={buttonRecommend} style={styleButtonItem} onClick={() => setDestinoEstado(true)}>
                                        <FontAwesomeIcon icon={faPeopleGroup} style={{ color: "#09b391" }} />
                                    </button>
                                    <button className={`button ${buttonRecommend? 'is-loading': ''}`} disabled={buttonRecommend} style={styleButtonItem} onClick={() => props.eliminarDelStack(props.idGrupo, itemId)}>
                                        <FontAwesomeIcon icon={faHeartBroken} style={{ color: "#e41b43", }} />
                                    </button>
                                    <button className={`button ${buttonRecommend? 'is-loading': ''}`} disabled={buttonRecommend} style={styleButtonItem} onClick={() => setModalEstado(true)}>
                                        <FontAwesomeIcon icon={faEye} style={{ color: props.item.rated === true ? "#277fdd" : "#e41b43" }} />
                                    </button>
                                </div>
                            </div>
                        </div>
