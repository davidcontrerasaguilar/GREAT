...
const EncuestaFinal = () => {
    ...
    useEffect(() => {
        const obtener_consenso = async () => {
            try {
                const resp_consensus = await api_consensus.get("/consensus-group/completeness", { params: { idSala } })
                if (resp_consensus) {
                    ...
                    const ranking = Object.entries(resp_consensus.data.completeness).sort(([, valueA], [, valueB]) => valueB - valueA);
