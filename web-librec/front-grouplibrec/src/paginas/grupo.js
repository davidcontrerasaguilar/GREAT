...
import { faArrowRightFromBracket, faPeopleGroup, faStar, faUser, faFlagCheckered, faTemperatureEmpty } from "@fortawesome/free-solid-svg-icons"
import PanelFavoritos from "../componentes/PanelFavoritos"
import { useTranslation } from "react-i18next"
import { buttonGroupContext, buttonIndividualContext, buttonRecommendContext } from "../context/ButtonContext.js"
...
const Providers = () => {
    ...
    const [buttonIndividual, setButtonIndividual] = useState(false)

    const [buttonRecommend, setButtonRecommend] = useState(false)

    return (
        <buttonGroupContext.Provider value={{buttonGroup, setButtonGroup}}>
            <buttonIndividualContext.Provider value={{buttonIndividual, setButtonIndividual}}>
                <buttonRecommendContext.Provider value={{buttonRecommend, setButtonRecommend}}>
                    <Grupo />
                </buttonRecommendContext.Provider>
            </buttonIndividualContext.Provider>
...
const Grupo = () => {
    ...
    const {buttonGroup, setButtonGroup} = useContext(buttonGroupContext)
    const {buttonIndividual, setButtonIndividual} = useContext(buttonIndividualContext)
    const {buttonRecommend, setButtonRecommend} = useContext(buttonRecommendContext)
    ...
    const ejecutarRecomendacionGrupo = async () => {
        ...
        setButtonGroup(true)
        try {
            const recomendacion_grupal = await api.get("/ejecutar-recomendacion-grupalv3", { params: { idGrupo } }, {
                headers: {
                    "Content-type": "application/json"
                }
            })
    ...
    const ejecutarRecomendacionIndividual = async () => {
        ...
        setButtonIndividual(true)
        try {
            const recomendacion_individual = await api.get("/ejecutar-recomendacion-individualv3", { params: { idUsuario, idGrupo } }, {
                headers: {
                    "Content-type": "application/json"
                }
            })
    ...
    const enviarAlStack = async (idItem) => {
        ...
        try {
            ...
            else if (resp.data.respuesta === "existe"){
                window.alert(`${t('main.alert.exists')}`)
            }
    ...
    const verificarCalificacionesFavoritos = async (e) => {
        try {
            ...
            if (verificar.data.respuesta === "stop") {
                let warning = ""
                if (!verificar.data.cantidad_calificados){
                    warning += `${t('main.alert.consensus.1', { max: verificar.data.maxFavoritos })}\n`
                }
                if (!verificar.data.todos_calificados){
                    warning += `${t('main.alert.consensus.2')}\n`
                }
                if (!verificar.data.minimo_calificados){
                    warning += `${t('main.alert.consensus.3')}\n`
                }
    ...
    return (
        ...
        <div style={stylePagina}>
            ...
            <div className="columns has-text-centered">
                ...
                <div className="column">
                    <button className={`button is-primary is-large is-rounded ${buttonIndividual? 'is-loading': ''}`} onClick={ejecutarRecomendacionIndividual}>
                ...
                { /* Recomendación Grupal solo líder */
                    ...
                    <div className="column">
                        <button className={`button is-primary is-large is-rounded ${buttonGroup? 'is-loading': ''}`} onClick={ejecutarRecomendacionGrupo}>
                ...
                <div className="column">
                    <Link to="/salas">
                        <button className={`button is-warning is-large is-right is-rounded`} onClick={salirGrupo} disabled={buttonGroup || buttonIndividual}>
            ...
            <div className="columns has-text-centered">
                ...
                <div className="column">
                    <button className={`button is-primary is-large is-rounded is-light`} onClick={verificarCalificacionesFavoritos} disabled= {buttonGroup || buttonIndividual}>
